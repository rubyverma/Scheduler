...
import com.scheduler.services.TimeslotService;
	...
	public class DepartmentController {
		...
		public String editNewDepartment(@PathVariable("departmentId") int departmentId, Model model) {
			...
			Departmenttimeslot slot = departmentTimeslotService.getDepartmentTimeslot(departmentId);
			
			dept.setCapacity(slot.getCapacity());
			char [] selectedDays = slot.getWeekdays().toCharArray();
			int i = 0;
			...
			dept.setDays(days);
			dept.setDepartmentTimeId(slot.getDepartmentTimeId());
		...
		public String saveDepartment(@ModelAttribute("department") Department department, Model model) {
			...
			if(department.getDepartmentId() == 0) {
				 ...
				 slot.setDepartmentId(departmentService.getSavedDepartmentById());
			} else {
				 i = departmentService.updateDepartment(department);
				 slot.setDepartmentId(department.getDepartmentId());
			}			
			
			slot.setCapacity(department.getCapacity());
			slot.setDepartmentTimeId(department.getDepartmentTimeId());
			...
			for (int j = 0; j < department.getDays().length; j++) {
				days[Integer.parseInt(department.getDays()[j]) - 1] = true;
			}
			...
			if(department.getDepartmentId() == 0) {
				...
				i = departmentTimeslotService.updateDepartmentTimeslot(slot);
			}
